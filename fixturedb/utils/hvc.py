...
def get_all_hvcs_referenced_by_targets(financial_years=None):
    ...
    :param financial_years: optional, you can manually define the financial years instead of getting them from the Target
    ...
    to_create = [
        HVCStruct(campaign_id=campaign_id, financial_year=int(str(financial_year)[-2:]))
                 for campaign_id in hvc_ids_expected_by_targets for financial_year in financial_years
    ]

    filter_q = reduce(
        operator.or_,
        [Q(campaign_id=data.campaign_id, financial_year=data.financial_year) for data in to_create]
    )
